...
def map_to_minus_pi_to_pi(angles):
    ...
    :param angles: A list of angles in rad.
    :return: A list of angle mapped to [-pi, pi].
    """
...
class Minitaur(object):
    ...
    Args:
      ...
      motor_overheat_protection: Whether to shut down the motor that has exerted
    ...
    """
        ...
        self._set_all_sensors(sensors if sensors is not None else list())
        self._is_safe = True
        ...
        self._reset(reset_time=reset_time)
    ...
    def step(self, action, control_mode=None):
        ...
        for i in range(self._action_repeat):
            proc_action = self.process_action(action, i)
            self._step_internal(proc_action, control_mode)
    ...
    def _reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
    ...
    """
        if reload_urdf:
            ...
            if self._on_rack:
                self.rack_constraint = (self._create_rack_constraint(
                    self._get_default_init_position(), self._get_default_init_orientation()))
            ...
            self._pybullet_client.resetBasePositionAndOrientation(
                self.quadruped, self._get_default_init_position(),
        ...
        self._is_safe = True
        self._last_action = None
        self._settle_down_for_reset(default_motor_angles, reset_time)
        if self._enable_action_filter:
    ...
    def _settle_down_for_reset(self, default_motor_angles, reset_time):
    ...
    """
        for i in range(self.num_legs):
            self._reset_pose_for_leg(i, add_constraint)

    def _reset_pose_for_leg(self, leg_id, add_constraint):
    ...
    def compute_motor_angles_from_foot_local_position(self, leg_id,
                                                      foot_local_position):
    ...
    def map_contact_force_to_joint_torques(self, leg_id, contact_force):
        ...
        jv = self.compute_jacobian(leg_id)
    ...
    def get_foot_position_in_base_frame(self):
        """Get the robot's foot position in the base frame."""
        assert len(self._foot_link_ids) == self.num_legs
        foot_positions = []
        for foot_id in self.get_foot_link_ids():
    ...
    """
        ...
        return map_to_minus_pi_to_pi(motor_angles)
    ...
    """
        return np.abs(np.dot(
            ...
            self.get_motor_velocities())) * self.time_step * self._action_repeat
    ...
    """
        ...
        orientation = self.get_true_base_orientation()
        return self.transform_angular_velocity_to_local_frame(angular_velocity,
                                                              orientation)
    ...
    def _apply_action(self, motor_commands, motor_control_mode):
    ...
    """
        ...
        q, qdot = self._get_pd_observation()
        qdot_true = self.get_true_motor_velocities()
        ...
        # GetMotorAngles and get_motor_velocities.
        ...
        self._set_motor_torque_by_ids(motor_ids, motor_torques)
    ...
    def get_base_masses_from_urdf(self):
        """Get the mass of the base from the URDF file."""
        return self._base_mass_urdf

    def get_base_inertias_from_urdf(self):
    ...
    def set_base_masses(self, base_mass):
    ...
    def set_base_inertias(self, base_inertias):
    ...
    def set_led_inertias(self, leg_inertias):
    ...
    def get_true_observation(self):
        ...
        observation.extend(self.get_true_motor_velocities())
        observation.extend(self.get_true_motor_torques())
        observation.extend(self.get_true_base_orientation())
        observation.extend(self.get_true_base_roll_pitch_yaw_rate())
        return observation
    ...
    def _get_pd_observation(self):
        pd_delayed_observation = self._get_delayed_observation(self._pd_latency)
    ...
    @staticmethod
    def _add_sensor_noise(sensor_values, noise_stdev):
        if noise_stdev <= 0:
    ...
    def set_control_letency(self, latency):
    ...
    def set_motor_strength_ration(self, ratio):
    ...
    def set_motor_strength_rations(self, ratios):
    ...
    def set_time_steps(self, action_repeat, simulation_step):
    ...
    """
        # If we want continuous resetting and is not the first episode.
        if self._reset_at_current_position and self._observation_history:
            _, _, yaw = self.get_base_roll_pitch_yaw()
    ...
    def _set_all_sensors(self, sensors):
    ...
    def _build_action_filter(self):
        ...
        num_joints = self.get_action_dimension()
